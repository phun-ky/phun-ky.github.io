  ...
  - **Safari's Intelligent Tracking Prevention (2017)** and **Firefox Enhanced Tracking Protection (2019)** block third-party cookies by default.`,
      ...
      timeline: [
        {
          ...
          year: '1990s',
          description:
            '"Webmasters" and "web designers" handled content, HTML, and servers all at once.',
        },
        {
          ...
          year: '2000s',
          description:
            '"Frontend developer" emerges as a distinct role as JS/CSS grow in complexity.',
        },
        {
    ...
    compliance (GDPR, CCPA) with usability, while avoiding **dark patterns** increasingly scrutinized by regulators.`,
        },
        {
          ...
          year: 'Today',
          description:
            "**Browser Privacy Features** - Safari's **Intelligent Tracking Prevention (ITP)**, Firefox's **Enhanced Tracking Protection (ETP)**, and Chrome's upcoming **Privacy Sandbox** phase-out of third-party cookies reshape how frontends handle analytics, ads, and personalization.",
        },
      ],
    },
  ],
};
const createHistory = (timeline) => {
  const slidesElement = document.getElementById('slides');

  if (!slidesElement || !timeline) return;
  ...
  chapters.forEach((chapter) => {
    ...
    const chapterContainerElement = document.createElement('section');
    const titleElement = document.createElement('div');

    titleElement.classList.add('sl-block');
    titleElement.style.setProperty('width', '1280px');
    titleElement.style.setProperty('left', '0px');
    titleElement.style.setProperty('top', '0px');
    titleElement.setAttribute('data-block-type', 'text');

    const titleContentElement = document.createElement('div');

    titleContentElement.classList.add('sl-block-content');

    const titleHeadingElement = document.createElement('h1');

    titleHeadingElement.style.setProperty('text-align', 'left');
    titleHeadingElement.append(title);
    titleContentElement.append(titleHeadingElement);
    titleElement.append(titleContentElement);

    chapterContainerElement.append(titleElement);

    if (description && description.length) {
      const descriptionElement = document.createElement('div');
      const descriptionContentElement = document.createElement('div');

      descriptionElement.classList.add('sl-block');
      descriptionElement.setAttribute('data-block-type', 'text');
      descriptionElement.style.setProperty('width', '1120px');
      descriptionElement.style.setProperty('left', '0');
      descriptionElement.style.setProperty('top', '246px');

      descriptionContentElement.classList.add('sl-block-content', 'paragraph');

      descriptionContentElement.innerHTML = markdownToHtml(description);
      descriptionElement.append(descriptionContentElement);
      chapterContainerElement.append(descriptionElement);
    }

    slidesElement.append(chapterContainerElement);
    ...
    timeline.forEach((item, itemIndex) => {
      ...
      const slideItemElement = document.createElement('section');
      const hasImage = imgURL.length !== 0;

      if (hasImage) {
        const imgContainerElement = document.createElement('div');
        const imgContentElement = document.createElement('div');

        imgContainerElement.classList.add('sl-block');
        imgContainerElement.style.setProperty('width', '600px');
        imgContainerElement.style.setProperty('max-height', '500px');
        imgContainerElement.style.setProperty('overflow', 'hidden');
        imgContainerElement.style.setProperty('top', '144px');
        imgContentElement.classList.add('sl-block-content');
        imgContainerElement.setAttribute('data-block-type', 'image');

        const imgElement = document.createElement('img');

        imgElement.setAttribute('src', imgURL);
        imgElement.classList.add('r-stretch');
        imgContentElement.append(imgElement);
        imgContainerElement.append(imgContentElement);
        slideItemElement.append(imgContainerElement);
      }

      const titleElement = document.createElement('div');

      titleElement.classList.add('sl-block');
      titleElement.style.setProperty('width', '640px');
      titleElement.style.setProperty('left', '640px');
      titleElement.style.setProperty('top', '144px');
      titleElement.setAttribute('data-block-type', 'text');
      ...
      titleHeadingElement.style.setProperty('text-align', 'left');
      titleHeadingElement.append(year);
      ...
      descriptionElement.classList.add('sl-block');
      descriptionElement.setAttribute('data-block-type', 'text');
      descriptionElement.style.setProperty('width', '640px');
      descriptionElement.style.setProperty('left', '640px');
      descriptionElement.style.setProperty('top', '288px');

      descriptionContentElement.classList.add('sl-block-content', 'paragraph');

      descriptionContentElement.innerHTML = markdownToHtml(description);
      descriptionElement.append(descriptionContentElement);
      slideItemElement.append(titleElement, descriptionElement);

      slidesElement.append(slideItemElement);
    });
  });
};
