...
export const PostsList = (posts,quantity = 20, excludeCategory) => {
  if(excludeCategory && typeof excludeCategory === 'string'){
    posts = posts.filter(post => excludeCategory.toLowerCase() !== post.category.toLowerCase());
  }
  ...
  if(posts.length > 20){
    ...
    <ul class="ph">${posts.slice(20, posts.length - 20).map(post => {
    const { title, url, year, month, day } = post;

    return html`<li class="ph">
    <a class="ph" href="${url}">${title}</a> – <time pubdate datetime="${year}-${month}-${day}" class="ph">${year}/${month}/${day}</time>
  </li>`;
  }).join('\n')}
</ul>`;
  }

  return html`<ul class="ph posts">${posts.slice(0, 20).map((post) => {
